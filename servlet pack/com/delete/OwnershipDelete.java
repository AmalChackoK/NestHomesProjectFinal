...
public class OwnershipDelete extends HttpServlet{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException{
		PrintWriter out=response.getWriter();
		 ArrayList<Ownershipbean> ownershipedit=null;
		 String oid=request.getParameter("osid");
		int osid=Integer.parseInt(oid);
		 try {
			ownershipedit=OwnershipDAO.fatchownership(osid);
...
+"</div>"
	...
	+"</div>"
		      	     ...
		      	     +" <form action='ownershipdeleteaction' method='post'>"
		      	    +"<div class='div-3'>"
		      	           +" <h4>Ownership</h4>");
		      	           for(Ownershipbean ow:ownershipedit) {
		      	        	   
		      	           
		      	             out.print("<input type='hidden' name='osid' value='"+ow.getOsid()+"'/>"
		      	            		 +"Owner"
		      	                +"<select  name='owner'>"

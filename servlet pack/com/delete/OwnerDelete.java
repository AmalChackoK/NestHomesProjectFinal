...
public class OwnerDelete extends HttpServlet{
	...
	PrintWriter out=response.getWriter();
	 ArrayList<Ownerbean> ownerdelete=null;
	 String ownerId=request.getParameter("oid");
	 int oid=Integer.parseInt(ownerId);
	 try {
		 ownerdelete=OwnerDAO.fetchownerById(oid);
...
+"</div>"
	...
	+"</div>"
         		+"<form action='ownerdeleteaction' method='Post'>"
         		+"<div class='div-2'>"
         			+"<h3>Owner Details</h3> <hr>");
         			for(Ownerbean ob:ownerdelete){
              			out.print("<input type='hidden' name='oid' value='"+ob.getOid()+"'/>"
              			+" name<br><input type='text' name='name' value='"+ob.getOname()+"' readonly/><br>"
              		 + "Address<br><textarea  name='address'  rows='4' cols='50'readonly/>"+ob.getOaddress()+"</textarea><br>"
              			+ "contact<br><input type='text' name='type' value='"+ob.getOphone()+"' readonly/><br>"

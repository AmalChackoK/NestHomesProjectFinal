...
public class UsageDelete extends HttpServlet{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException{
		PrintWriter out=response.getWriter();
		 ArrayList<Usagebean> usagebean=null;
		
		 int guid=Integer.parseInt(request.getParameter("guid"));
		 
		 try {
			 usagebean=GasusageDAO.fatchusage(guid);
...
+"</div>"
	...
	+"</div>"
		      	     ...
		      	     +" <form action='usagedeleteaction' method='post'>"
		      	    +"<div class='div-2'>"
		      	           +" <h4>gas usage</h4>");
		      	           for(Usagebean ow:usagebean) {
		      	        	   
		      	           
		      	             out.print("<input type='hidden' name='guid' value='"+ow.getGuid()+"'/>"
		      	            		 +"flate"
		      	                +"<select  name='flate'>"
		      	                +"<option >select flate</option>");
		ArrayList<Flatbean>  owner=null;    	                
		      try{
		    	  owner=FlatDAO.listAllflats();
		      }catch (SQLException sql) {
				sql.printStackTrace();
			}
		     for(Flatbean ob:owner){
		    	 out.print("<option value='"+ob.getFid()+"'>"+ob.getFnumber()+""+ob.getFtower()+"</option><br>");
		     }
		      	                
		 	         
		     out.print(" </selcet><br>"
		     		+ "<br>unit <br><input type='text' name='unit' value='"+ow.getUsage()+"' /><br>"

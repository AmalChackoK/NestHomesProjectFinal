...
public class UserDelete extends HttpServlet{
 ...
 public void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException{
	 ...
	 ArrayList<Userbean> userdelete=null;
	 String usrId=request.getParameter("uid");
	 int uid=Integer.parseInt(usrId);
	
	 try {
		 userdelete=UserDAO.fetchuserById(uid);
...
+"</div>"
	...
	+"</div>"
       		...
       		+"<div class='div-2'>"
       			+"<h3>User</h3> <hr>");
       		for(Userbean ob:userdelete){
       			out.print("<input type='hidden' name='uid' value='"+ob.getUid()+"'/>"
       			+" Username<br><input type='text' name='username' value='"+ob.getUname()+"' readonly /><br>"
       			+"Password<br><input type='text' name='password' value='"+ob.getUpass()+"' readonly/><br>"
       			+ "Type<br><input type='text' name='type'value='"+ob.getUtype()+"' readonly/><br>"

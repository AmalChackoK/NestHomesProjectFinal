...
public class TenantEdit extends HttpServlet{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException{
		PrintWriter out=response.getWriter();
		 ArrayList<Tenantbean> tenantedit=null;
		 String tenantId=request.getParameter("tid");
		 int tid=Integer.parseInt(tenantId);
		 try {
			 tenantedit=TenantDAO.fetchtenantById(tid);
...
+"</div>"
	...
	+"</div>"
	          		+"<form action='tenanateditaction' method='Post'>"
	          		+"<div class='div-2'>"
	          			+"<h3>Tenant Details</h3> <hr>");
	          			for(Tenantbean ob:tenantedit){
	               			out.print("<input type='hidden' name='tid' value='"+ob.getTid()+"'/>"
	               			+" name<br><input type='text' name='name' value='"+ob.getTname()+"' /><br>"
	               		 + "Address<br><textarea  name='address'  rows='4' cols='50'/>"+ob.getTaddress()+"</textarea><br>"
	               			+ "contact<br><input type='text' name='phone' value='"+ob.getTphone()+"' /><br>"

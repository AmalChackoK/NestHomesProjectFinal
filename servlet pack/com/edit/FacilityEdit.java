...
public class FacilityEdit extends HttpServlet{
...
+"</div>"
	...
	+"</div>"
	          		+"<form action='facilityeditaction' method='Post'>"
	          		+"<div class='div-2'>"
	          			+"<h3>Owner Details</h3> <hr>");
	          			for(Facilitybeen ob:facilityedit){
	               			out.print("<input type='hidden' name='fmid' value='"+ob.getFmid()+"'/>"
	               					+" name<br><input type='text' name='name' value='"+ob.getFmname()+"' /><br>"
	               					+"Designation<br><input typ='text' name='des' value='"+ob.getFmdes()+"'><br>"
	               					+ "Duty<br><input type='text' name='Dauty' value='"+ob.getFmduty()+"'/><br>"
	               					+ "contact<br><input type='text' name='phone' value='"+ob.getFmphone()+"' /><br>"
			...
			for(Facilitybeen ob:user){
				out.print("<tr>"
						...
						+ "<td><a href='ownerdelete?oid="+ob.getFmid()+"'>Delete</a></td>"
						+"</tr>");
				i++;
			}
			out.print("</table>"

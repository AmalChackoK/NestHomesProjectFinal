...
public class FlatEdit extends HttpServlet{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException{
		 ...
		 String flatId=request.getParameter("fid");
		 System.out.println(flatId);
		 int fid=Integer.parseInt(flatId);
		 try {
...
+"</div>"
	...
	+"</div>"
	          		+"<form action='flatdeleteaction' method='Post'>"
	          		+"<div class='div-2'>"
	          			+"<h3>flates</h3> <hr>");
	          			for(Flatbean ob:flatedite){
	               			out.print("<input type='hidden' name='fid' value='"+ob.getFid()+"'/>"
	               			+" Number<br><input type='text' name='Number' value='"+ob.getFnumber()+"' /><br>"
	               			+"Tower<br><input type='text' name='Tower' value='"+ob.getFtower()+"'/><br>"
	               			+ "type<br><input type='text' name='type' value='"+ob.getFtype()+"' /><br>"
			...
			for(Flatbean ob:user){
				out.print("<tr>"
						...
						+ "<td><a href='flatdelete?fid="+ob.getFid()+"'>Delete</a></td>"
						+"</tr>");
				i++;
			}
			out.print("</table>"
